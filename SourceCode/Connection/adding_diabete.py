...
def adding_diabete(n1=n1,n2=n2,path_consumer=None,path_source=None ):
    ...
    while( j != n2 ):
        ...
        for i in range(len(source)):
             ...
             if (  consumer_name == source[i][1] and consumer_cod.find(str(source[i][0]))!=-1 ):
                print( "Find : {0} - {1}                     \r".format(source[i][0],source[i][1]) )
                sheetConsumer.Cells(j,diabete_consumer).value = source[i][6]
                source.pop(i)
                break
        j+=1
